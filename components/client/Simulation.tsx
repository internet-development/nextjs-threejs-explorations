...
const Simulation: React.FC<SimulationProps> = ({ cloudColorHex, skyColorHex, backgroundImageURL, backgroundVertexShader, backgroundFragmentShader, children, windEnabled }) => {
  ...
  useEffect(() => {
    ...
    const simulationCallbacks: ((time: number) => void)[] = [];
    ...
    let shaderUniforms: { [uniform: string]: { value: any } } | undefined;
    ...
    const animate = (time: number) => {
      ...
      if (backgroundVertexShader && backgroundFragmentShader && backgroundScene && backgroundCamera) {
        if (shaderUniforms) shaderUniforms.iTime.value = time * 0.001;
  ...
  }, [backgroundImageURL, backgroundVertexShader, backgroundFragmentShader, windEnabled]);
  ...
  if (backgroundImageURL && !(backgroundVertexShader && backgroundFragmentShader)) {
    styles = {
      ...styles,
